...
# Make sure the script is not being executed with superuser priviledges.
if [[ "${UID}" -eq 0 ]]
then
...
# Remove the options while leaving the remainding arguments.
shift "$(( OPTIND - 1 ))"
...
# Anything that remains on the command line it's to be treated as a single command.
COMMAND="${@}"
